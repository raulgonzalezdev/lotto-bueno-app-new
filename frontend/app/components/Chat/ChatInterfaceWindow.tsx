...
const ChatInterfaceComponent: React.FC<ChatInterfaceComponentProps> = ({
  ...
  useEffect(() => {
    const sessionId = getSessionId(); // Initialize SESSION_ID
    console.log('Session ID on mount:', sessionId);
    ...
    try {
      await fetch("/api/conversations", {
        ...
        body: JSON.stringify({
          ...
          messages: messages.map(message => ({ ...message, session_id: sessionId })),
        }),
      });
    } catch (error) {
    ...
    for (const chunk of chunks) {
      try {
        const response = await fetch("/api/save_chunk", {
          ...
          body: JSON.stringify({ ...chunk, session_id: sessionId }),
        });

        if (!response.ok) {
          const data = await response.json();
          console.error("Error saving chunk:", data);
        }
      } catch (error) {
  ...
  const saveContextToDatabase = async (context: string) => {
    const sessionId = getSessionId();
    try {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if (sendInput.trim()) {
      setMessages((prev) => [...prev, { type: "user", content: sendInput, session_id: sessionId }]);
      setUserInput("");
      ...
      try {
        ...
        const response = await fetch(APIHost + "/api/query", {
          ...
          body: JSON.stringify({ query: sendInput, session_id: sessionId }),
        });
        ...
        if (data) {
          ...
          saveChunksToDatabase(data.chunks);
          setSuggestions([]);
          ...
          if (data.context) {
            ...
            saveContextToDatabase(data.context);
  ...
  return (
    <div className="flex flex-col gap-2">
      {/*Chat Messages*/}
      <div className="flex flex-col bg-bg-alt-verba rounded-lg shadow-lg p-2 md:p-5 text-text-verba gap-5 h-[45vh] overflow-auto">
        ...
        {isFetching && (
          <div className="flex items-center pl-4 mb-4 gap-3">
            ...
            />
            <p>
              {fetchingStatus === "CHUNKS" && "Recuperando fragmentos"}
              {fetchingStatus === "RESPONSE" && "Generando Respuesta"}
            </p>
          </div>
        )}
      </div>
